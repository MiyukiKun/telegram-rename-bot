...
import os
from helper import Timer, progress_bar_str
...
async def rename(event):
    ...
    async def callbackd(current, total):
        if timer.can_send():
            await reply.edit(f"Downloading...\n{progress_bar_str(current, total)}")
    async def callbacku(current, total):
        if timer.can_send():
    ...
    try:
        ...
        await bot.download_media(msg, name, progress_callback=callbackd)
        file = await bot.upload_file(name, progress_callback=callbacku)
